...
All modules follow a consistent **kwargs pattern** for initialization:
- **Required dependencies are explicit positional arguments** (e.g., `database`, `vault`)
- **All optional configuration via `**kwargs`** (e.g., `bot_token`, `port`, `session_secret`)
- Module reads its own configuration from Vault in `__init__` with fallback to kwargs
- Benefits: clean signatures, easy extension, forces required dependencies

Example:
```python
def __init__(self, database: object, vault: object, **kwargs):
    self.database = database
    self.vault = vault
    
    # Extract kwargs with defaults
    port = kwargs.get('port', 8080)
    users_client = kwargs.get('users_client')
    
    # Read module-specific config from Vault
    config = self.vault.kv2engine.read_secret(path='configuration/module-name')
    self.setting = config.get('setting', kwargs.get('setting', 'default'))
```
