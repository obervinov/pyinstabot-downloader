...
def fixture_webui_setup(database_class, vault_instance, users_client, users_client_rl):
    ...
    webui = WebUI(
        ...
        users={'auth': users_client, 'rate_limited': users_client_rl},
        bot_token=bot_token,
    ...
    token = users_client.issue_token(user_id="test_user_1")
    return {"webui": webui, "client": client, "bot_token": bot_token, "token": token}
...
def test_webui_login_page(webui_setup):
    ...
    assert "Login with Token" in response.text
...
def test_webui_token_auth(webui_setup):
    client = TestClient(webui_setup["webui"].app)
    token = webui_setup["token"]

    response = client.post("/auth/token", data={"token": token}, allow_redirects=False)
...
def test_webui_token_invalid(webui_setup):
    client = TestClient(webui_setup["webui"].app)
    response = client.post("/auth/token", data={"token": "invalid.token"}, allow_redirects=False)
    assert response.status_code == 401
