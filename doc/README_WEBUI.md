...
- **Authentication**: Telegram Login Widget with HMAC-SHA256 verification or one-time tokens issued from the bot
...
users_client = Users(vault={'instance': vault, 'role': 'pyinstabot-downloader-users'}, rate_limits=False)
...
- `POST /auth/token` - Token-based login (one-time tokens from /webui_login)
...
1. User runs `/webui_login` in Telegram bot → receives one-time token (TTL from Vault `configuration/webui.token-ttl`, default 10 minutes if missing)
2. User opens WebUI `/` and pastes token into the "Login with Token" form
3. WebUI calls `users.validate_token()` → verifies token hash/expiry, marks token as used
4. Allowed user stored in session → redirect to `/dashboard`
