...
class DropboxDownloader:
    ...
    def __init__(self,
        ...
        try:
            ...
            self.dropbox_object = dropbox.Dropbox(
                                        dropbox_token,
            ...
            for f in files:
                ...
                if ".txt" in filename[len_filename-1]:
                    ...
                    with open(f, 'rb') as file_transfer:
                        try:
                            upload_file = self.dropbox_object.files_upload(
                                                file_transfer.read(),
                                                dropbox_filename,
                                                autorename=True
                                                )
                            log.info(
                                ...
                                f"file {upload_file.name} has been uploaded"
                                )
                            ...
                            log.error(
                                f"[class.{__class__.__name__}]"
                                f"uploading picture to dropbox api faild: {ex}"
                                )
                            break
