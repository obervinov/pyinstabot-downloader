...
class InstagramDownloader:
    ...
    def __init__(self, Vault = None, user: Str = None, password: Str = None, sessionfile: Str = None, bot_name: Str = None, Dropbox = None, Telebot = None) -> None:
    ...
    def get_posts_list(self, username: str = None):
        ...
        try:
            ...
            log.info(f"[class.{__class__.__name__}] reading list of posts from account {username} was successful")
            ...
            if "InvalidPath" not in shortcodes:
               # Checking the status of already uploaded posts
                log.info(f"[class.{__class__.__name__}] excluding shortcodes already downloaded ...")
                for key, value in shortcodes.items():
                  if value == 'success':
                    shortcodes_list_exist.append(key)
                
                # Exclusion from the list of posts received from instagram successfully uploaded posts
                log.info(f"[class.{__class__.__name__}] building list of shortcodes for downloaded ...")
                for post in posts: 
                  if post.shortcode not in shortcodes_list_exist:
                    shortcodes_list.append(post.shortcode)

            log.info(f"[class.{__class__.__name__}] building list of posts for downloaded  was be done")
            return shortcodes_list

        except Exception as ex:
            log.error(f"[class.{__class__.__name__}] readed posts from account {username} faild {ex}")
    ...
    def download_post(self, shortcode: str = None):
        ...
        try:
            ...
            log.info(f"[class.{__class__.__name__}] content of post {shortcode} successful downloaded in temp storage")
            ...
            log.error(f"[class.{__class__.__name__}] content by post {shortcode} faild downloaded: {ex}")
            self.Vault.vault_put_secrets(vault_path, shortcode, 'faild')




    def download_all_posts(self, username: str = None, ratelimit_timeout: int = 30, ratelimit_max_timeout: int = 1800, chat_id: str = None):
        # Unchangeable minimum timeout value (to be reset to default value when ratelimit_max_timeout is reached)
        ...
        self.Telebot.send_message(chat_id, f"\u2B50 Reading posts from account <b>{username}</b> successful.\n\U0001F303 Posts count: <b>{self.posts_count}</b>\n\U0001F6A6 Ratelimit timings: <i>min {ratelimit_timeout}s - max {ratelimit_max_timeout}s</i>")
        ...
        for shortcode in shortcodes_list:
            ...
            # checking the condition whether the first message with statistics has already been sent for its subsequent editing 
            ...
            log.warning(f"[class.{__class__.__name__}] ratelimit aplied in {datetime.datetime.now().strftime('%H:%M:%S')}: {ratelimit_timeout}")
        ...
        log.info(f"[class.{__class__.__name__}] all available posts from account {username} has been downloaded")
