...
* `.github/workflows`: grant `pr.yaml` and `release.yaml` the full permission set that `_templates/.github/workflows/docker.yaml` declares on its job. A called workflow only gets what its caller granted, and GitHub rejects the whole run at startup when it asks for more — with no log and no check to point at. Both workflows failed to start from the v4.0.0 bump onwards, which is why v3.3.7 was merged without a release and without a release image.
#### 📚 Documentation
* `README.md`: refresh the stale references — the vault environment variables link pointed at `vault-package@v3.0.0`, the roles anchor at the wrong lines of `constants.py`, and one command still used the removed `docker-compose` v1 binary.
...
* `README.md`: read the Python version from `pyproject.toml` instead of hardcoding `3.12.7`, and drop the tag from the `vault-package` environment variables link so it follows that repository's default branch.
...
* [Bug: Critical error when rotating credentials from the database](https://github.com/obervinov/pyinstabot-downloader/issues/118)
#### 🚀 Features
* bump base image to `python:3.12.9`
